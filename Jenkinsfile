...
podTemplate(label: 'k8stcpmap-controller',
  containers: [
    containerTemplate(
      name: 'golang',
  ...
  node ('k8stcpmap-controller') {
    ansiColor("xterm") {
      ...
      stage("build") {
        ...
        container('helm') {
          sh 'helm lint chart/k8stcpmap-controller'
        }
      }
      ...
      stage("publish") {
        if (!env.TAG_NAME) {
          ...
          container(name: 'kaniko', shell: '/busybox/sh') {
            ...
            sh "/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --destination='nexus.doodle.com:5000/devops/k8stcpmap-controller:${env.TAG_NAME}'"
          }
          ...
          container('helm') {
            ...
            tgz="k8stcpmap-controller-${version}.tgz"
            sh "mkdir chart/k8stcpmap-controller/crds"
            sh "cp config/crd/bases/* chart/k8stcpmap-controller/crds"
            sh "helm package chart/k8stcpmap-controller"
          }
...
def bumpImageVersion(String version) {
  ...
  def valuesFile = "./chart/k8stcpmap-controller/values.yaml"
...
def bumpChartVersion(String version) {
  ...
  def chartFile = "./chart/k8stcpmap-controller/Chart.yaml"
